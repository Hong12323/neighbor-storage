...
export default function SettingsScreen() {
  ...
  const [riderMode, setRiderMode] = useState(user?.isRider || false);

  const riderMutation = useMutation({
    mutationFn: async (value: boolean) => {
      const res = await apiRequest("PATCH", "/api/users/me/rider", { isRider: value });
      return res.json();
    },
    onSuccess: (data) => {
      setRiderMode(data.isRider);
      showToast(data.isRider ? "라이더 모드 활성화!" : "라이더 모드 비활성화", "info");
    },
    onError: () => showToast("변경 실패", "error"),
  });
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {/* Rider Section */}
        <View style={styles.section}>
          <Text style={styles.sectionLabel}>이웃 라이더</Text>
          <View style={styles.sectionCard}>
            <View style={styles.settingsRow}>
              <View style={styles.settingsRowLeft}>
                <MaterialCommunityIcons name="moped" size={18} color={Colors.primary} />
                <View style={{ flex: 1 }}>
                  <Text style={styles.settingsLabel}>라이더 모드</Text>
                  <Text style={styles.riderDesc}>배달 요청을 받아 수익을 얻을 수 있어요</Text>
                </View>
              </View>
              {riderMutation.isPending ? (
                ...
                <Switch
                  value={riderMode}
                  onValueChange={(v) => riderMutation.mutate(v)}
            ...
            {riderMode && (
              <>
                <View style={styles.divider} />
                <SettingsRow
                  icon={<Ionicons name="bicycle-outline" size={18} color={Colors.primary} />}
                  label="라이더 대시보드"
                  accent
