...
import {
  StyleSheet, Text, View, ScrollView, TextInput, Pressable, Platform, Switch, ActivityIndicator,
} from 'react-native';
    ...
    try {
      if (Platform.OS === 'web') {
        await new Promise<void>((resolve, reject) => {
          navigator.geolocation.getCurrentPosition(
            (pos) => {
              ...
              if (!locationAddress) setLocationAddress('현재 위치');
              resolve();
            },
        ...
        try {
          ...
          if (geocode.length > 0) {
            const g = geocode[0];
            const addr = [g.city, g.district, g.street, g.name].filter(Boolean).join(' ');
            if (addr) setLocationAddress(addr);
            else if (!locationAddress) setLocationAddress('현재 위치');
          }
        } catch {
          if (!locationAddress) setLocationAddress('현재 위치');
        }
      }
      showToast('현재 위치를 가져왔습니다', 'success');
    } catch {
      ...
      >
        ...
        <View style={styles.field}>
          <Text style={styles.fieldLabel}>
            <Ionicons name="location" size={14} color={Colors.error} /> 거래 희망 장소
          </Text>
          <View style={styles.locationRow}>
            <TextInput
              style={[styles.input, { flex: 1 }]}
              placeholder="예: 오전역 1번 출구, 이마트 앞"
              placeholderTextColor={Colors.textLight}
          ...
          <Text style={styles.locationHint}>
            {locationLat && locationLng
              ? `📍 좌표 저장됨 (${locationLat.toFixed(4)}, ${locationLng.toFixed(4)})`
              : '📌 아이콘을 눌러 현재 위치를 자동 입력'}
          </Text>
...
const styles = StyleSheet.create({
  ...
  locationHint: {
    fontFamily: 'NotoSansKR_400Regular', fontSize: 11,
    color: Colors.textLight, marginTop: 6,
  },
  mapPreview: { marginTop: 10, borderRadius: 14, overflow: 'hidden', position: 'relative' },
  clearLocBtn: {
